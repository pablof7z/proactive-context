//! recall usage accounting — accumulate token/cost/cache stats across a REPL
//! session, per model, and render a compact statusbar + a detailed `/usage` view.

use std::collections::BTreeMap;

/// Per-call usage returned by the LLM layer.
#[derive(Default, Clone)]
pub struct Usage {
    pub prompt_tokens: u64,
    pub completion_tokens: u64,
    pub cached_tokens: u64, // prompt tokens served from cache (OpenRouter reports this)
    pub cost: f64,          // USD; 0.0 when the provider doesn't report cost (e.g. Ollama)
    pub cost_known: bool,
}
