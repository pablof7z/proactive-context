...
- **Transcript-enriched injection queries.** The user asked whether the injection hook should query using *conversation context*, not just the bare current prompt (so a terse follow-up like "what about retry logic?" searches with the preceding context). This is genuinely useful for retrieval quality — but it adds work to the **hot path** (read + parse `transcript_path`, build a larger query, larger embed) on *every* prompt. v0.2 currently specs injection as v0.1 behavior (embed the bare prompt). **Decision needed:** keep injection bare-prompt for now, or fold in the last N turns (and if so, how many, and how merged — concatenate vs. summarize)? Recommendation: ship v0.2 bare-prompt, add transcript-enriched querying as a fast-follow with a measured latency budget.
